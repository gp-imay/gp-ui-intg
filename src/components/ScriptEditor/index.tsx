...
export function ScriptEditor({ scriptId }: ScriptEditorProps) {
  ...
  useEffect(() => {
    ...
    async function fetchScriptSegments() {
      try {
        ...
        // Set title regardless of whether we have segments or not
        setTitle(`Script ${scriptId.slice(0, 8)}`);
        ...
        if (data.segments && data.segments.length > 0) {
          ...
          // Keep the default empty element
        }
    ...
    fetchScriptSegments();
  }, [scriptId, showAlert, hasAttemptedLoad]);
